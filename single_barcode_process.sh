...
JOBID4=$(sbatch \
   --dependency=afterok:$JOBID3 \
    ...
    --wrap "
    # Append percentage retained to  file
    echo 'Barcode_${barcode_number}:' \"\$(cat \"barcode${barcode_number}/${barcode_number}_barcode_percent_retained.txt\")\" >> \"percent_reads_retained_length_filter.txt\"

    # Append number of fail reads to file
    echo 'Barcode_${barcode_number}:' \"\$(cat \"barcode${barcode_number}/${barcode_number}_num_fail.txt\")\" >> \"no_fail_reads.txt\"

    # Append number of ignored reads to file
    echo 'Barcode_${barcode_number}:' \"\$(cat \"barcode${barcode_number}/${barcode_number}_number_ignored_reads.tsv\")\" >> \"no_reads_ignored_parse_filter.txt\"

    # Append the taxaID_count file to all taxaID_count file
    echo \"\$(cat \"barcode${barcode_number}/${barcode_number}_taxaID_counts.tsv\")\" >> \"all_taxaID_count.tsv\"")
